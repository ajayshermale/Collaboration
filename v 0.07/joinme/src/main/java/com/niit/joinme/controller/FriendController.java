...
public class FriendController {
	...
	@RequestMapping(value="/acceptFriendRequest/{user_id}", method=RequestMethod.PUT)
	public ResponseEntity<Void> update(@PathVariable("user_id")Integer user_id){
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
	      String userName = authentication.getName();
	   //   int friendId = userService.getByUsername(username).getuser_id();
	      int friendId = userService.loadUserByUsername(userName).getUserId();
	      friendService.update(friendId, user_id);
		return new ResponseEntity<Void>(HttpStatus.OK);
		
	}
	
	@RequestMapping(value="/rejectFriendRequest/{user_id}", method=RequestMethod.PUT)
	public ResponseEntity<Void> reject(@PathVariable("user_id")Integer user_id){
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
	      String userName = authentication.getName();
	      int friendId = userService.loadUserByUsername(userName).getUserId();
	      friendService.reject(friendId, user_id);
		return new ResponseEntity<Void>(HttpStatus.OK);
